 ...
 * horario do NETPA (rapida) ou a escolha a mao (sempre fiavel). O que a captura
...
export default function EnrollFlow({ onClose, onSaved, onStart, preSelect = [], preTurnos = {} }) {
  ...
  const { rows: courses, add: addCourse } = useCourses()
  const { t } = useT()

  const ano = Number(academicYear) || null
  const termo = Number(semester) || null
  // Abrindo pelo Horario ja ha escolhas feitas: salta a apresentacao.
  const [passo, setPasso] = useState(preSelect.length ? 'escolher' : 'intro')
  const [escolhidas, setEscolhidas] = useState(preSelect)
  const [turnos, setTurnos] = useState(preTurnos)
   ...
   * escolhidos e tira os que o aluno desmarcou.
   *
   * So mexe em blocos com titulo de turno oficial E de cadeiras que estao nesta
   * escolha — o que ele escreveu a mao, e as cadeiras que nem abriu, ficam
   * intactos. Cada passo e idempotente, por isso repetir depois de uma falha a
  ...
  async function guardar() {
    ...
    try {
      const idPorCodigo = {}
      for (const c of courses) if (c.code) idPorCodigo[String(c.code)] = c.id

      for (const [i, code] of escolhidas.entries()) {
        const ficha = porCodigo[code]
        if (!ficha || idPorCodigo[code]) continue
        ...
        idPorCodigo[code] = criada.id
      }

      const { data: existentes, error } = await supabase
        .from('schedule_blocks').select('id, title, day_of_week, start_time')
        .eq('user_id', user.id)
      if (error) throw error

      const chave = (b) => `${b.title}|${b.day_of_week}|${String(b.start_time).slice(0, 5)}`
      const querido = escolhidas.flatMap((code) => {
        const ficha = porCodigo[code]
        if (!ficha) return []
        return blocksFor(code, ficha.name, turnos[code])
          .map((b) => ({ ...b, course_id: idPorCodigo[code] || null, user_id: user.id }))
      })

      const jaLa = new Set((existentes || []).map(chave))
      const aInserir = querido.filter((b) => !jaLa.has(chave(b)))
      if (aInserir.length) {
        const { error: e } = await supabase.from('schedule_blocks').insert(aInserir)
        if (e) throw e
      }

      const queridos = new Set(querido.map(chave))
      const aApagar = (existentes || []).filter((b) => {
        const info = officialBlock(b.title)
        return info && escolhidas.includes(info.code) && !queridos.has(chave(b))
      })
      for (const b of aApagar) {
        const { error: e } = await supabase.from('schedule_blocks').delete().eq('id', b.id)
        if (e) throw e
      }
    ...
    <Modal open onClose={onClose} title={titulo}>
      ...
      {passo === 'intro' && (
        <>
          ...
          <button onClick={() => { onStart?.(); setPasso('escolher') }}
          ...
          <button onClick={onClose} className="w-full text-center text-sm text-slate-400 mt-5 py-2">
      ...
      {passo === 'escolher' && (
        <>
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={onClose} className="btn-ghost flex-1 py-2.5 text-sm">{t('coursesPrompt.later')}</button>
