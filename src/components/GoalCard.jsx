...
export default function GoalCard({ current, goal, onSave, subtitle, compact = false }) {
  ...
  if (editing) {
    return (
      <div className="card p-4">
        <p className="text-sm font-semibold text-slate-200 mb-1">🎯 Objetivo de média deste semestre</p>
        <p className="text-xs text-slate-500 mb-3">Que média (0–20) queres atingir este semestre?</p>
        <div className="flex items-center gap-2">
          <input autoFocus type="number" step="0.5" min="0" max="20" inputMode="decimal"
            className="input w-24 text-lg font-semibold" placeholder="ex. 15"
          ...
          {goal != null && (
            <button onClick={async () => { setDraft(''); setSaving(true); try { await onSave(null); setEditing(false) } finally { setSaving(false) } }}
              disabled={saving} className="btn-ghost px-3 py-2 text-sm text-rose-400">Remover</button>
          )}
          <button onClick={() => setEditing(false)} disabled={saving} className="btn-ghost px-3 py-2 text-sm">Cancelar</button>
          <button onClick={save} disabled={saving} className="btn-primary px-4 py-2 text-sm">Guardar</button>
        </div>
      </div>
    )
  }
    ...
    return (
      <button onClick={startEdit}
        ...
        <span className="flex-1 min-w-0">
          <span className="block text-sm font-semibold text-slate-100">Define um objetivo de média</span>
          <span className="block text-xs text-slate-400">Segue quão perto estás da tua meta do semestre.</span>
        </span>
  ...
  const st = goalStatus(current, goal)
  const tone = TONE[st.tone]
  return (
    <div className="card p-4">
      <div className="flex items-center gap-2 mb-2.5">
        <span className="text-sm font-semibold text-slate-200">🎯 Objetivo do semestre</span>
        {subtitle && <span className="text-xs text-slate-500">· {subtitle}</span>}
        <div className="flex-1" />
        <button onClick={startEdit} className="p-1 text-slate-500 hover:text-slate-200" aria-label="Editar objetivo">
