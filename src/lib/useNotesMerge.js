...
import { isPlanRow, isSummaryRow, isNoteRow, isReviewRow, isHandbookRow } from './plan.js'
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        for (const n of data || []) {
          if (isPlanRow(n) || isSummaryRow(n) || isNoteRow(n) || isReviewRow(n) || isHandbookRow(n)) continue
